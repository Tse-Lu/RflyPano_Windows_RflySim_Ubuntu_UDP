...
desired_speed = 2 # flight velocity
takeoff_speed = 2 
dt = 0.05        # control period
T_total = 50    # total time
omega = desired_speed / radius
...
for i in range(VehilceNum):
    ...
    while t < T_total:
        target_x = radius * np.cos(t) + x0 - radius
        target_y = radius * np.sin(t) + y0
        
        target_yaw = t + np.pi/2 
        
        MavList[i].SendPosNED(target_x, target_y, -height, target_yaw)
